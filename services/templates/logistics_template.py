...
def get_logistics_template():
    ...
    return """
        ...
        {% for row in results %}
            {# Check both _document_type field and field-based detection #}
            {% if row.get('_document_type') == 'fta_list' or row.get('FTAAgreement') or row.get('CountryOfOrigin') %}
                {% set ns.has_fta = true %}
            {% elif row.get('_document_type') == 'bill_of_lading' or row.get('waybill_type') or row.get('bol_number') or row.get('BLNumber') or row.get('shipper') %}
                {% set ns.has_bol = true %}
            {% elif row.get('_document_type') == 'packing_list' or row.get('CartonNumber') or row.get('carton_number') %}
        ...
        {% if ns.has_bol or (results and (results[0].get('waybill_type') or results[0].get('bol_number') or results[0].get('BLNumber') or results[0].get('shipper'))) %}
        <table style="width: 100%; border-collapse: collapse; margin-top: 20px; background: white;">
            <thead>
                <tr style="background-color: #0B1221; color: white;">
            ...
            <tbody>
            {% for row in results %}
                {% if row.get('_document_type') == 'bill_of_lading' or row.get('waybill_type') or row.get('bol_number') or row.get('BLNumber') %}
            <tr style="border-bottom: 1px solid #ddd;">
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('BLNumber', row.get('bol_number', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('Shipper', row.get('shipper', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('Consignee', row.get('consignee', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('Vessel', row.get('vessel', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('ContainerNumber', row.get('container_number', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('PortOfLoading', row.get('port_of_loading', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('PortOfDischarge', row.get('port_of_discharge', 'N/A')) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('CargoDescription', row.get('cargo_description', row.get('Description', 'N/A'))) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd;">{{ row.get('GrossWeight', row.get('gross_weight_kg', row.get('gross_weight', row.get('Weight', 'N/A')))) }}</td>
                <td style="padding: 10px; border: 1px solid #ddd; font-size: 11px;">{{ row.get('Filename', 'N/A') }}</td>
            </tr>
                {% endif %}
            {% endfor %}
