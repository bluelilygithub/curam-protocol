...
def generate_roadmap(industry_config, doc_staff_count, hours_per_week, avg_rate):
    ...
    for task in tasks:
        ...
        roadmap.append({
            ...
            'automation_potential': import os
import base64
import requests
from flask import render_template_string, request, session, send_file, Response, url_for, redirect
import pandas as pd
    ...
    <div class="container">
        ...
        <div style="background: linear-gradient(135deg, #0B1221, #1a2332); border-radius: 16px; padding: 3rem 2rem; margin: 3rem 0; text-align: center;">
            ...
            <div class="cta-grid" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem; max-width: 700px; margin: 0 auto;">
                ...
                <!-- Book Phase 1 Button -->
                <a href="{{ url_for('contact', option='phase-1') }}" class="btn-primary-huge" style="background: linear-gradient(135deg, #D4AF37, #B8941F); color: #0B1221; padding: 1.25rem 2rem; font-size: 1.1rem; display: flex; flex-direction: column; align-items: center; gap: 0.5rem; text-decoration: none; border: 2px solid #FFD700;">
                    <span style="font-size: 1.5rem;">🚀</span>
...
# Example: app.register_blueprint(roi_app, url_prefix='/roi-calculator')automation_potential,
            'success_rate_decimal': success_rate,
            'conservative_estimate': conservative_estimate,
            'potential': task.get('potential', 'MEDIUM')
        })
