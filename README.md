...
| **Wellbeing & Personality Checks** | Proof-of-concept self-report area behind the heart-pulse nav. Eight checks are grouped into bordered modules: Mood & Emotional State (BDI, GAD-7, PANAS), Personality & Traits (IPIP-NEO-120, HEXACO-60), and Regulation & Coping (GAD-7, ASRS-5, CERQ, Brief COPE). Each module can generate a cached report and its own PowerPoint slideshow; final Summary, Detailed, Analytical, Suggestions, and final recap slideshows are generated from the module outcomes. Slideshows include summary chart slides, individual test findings, final synthesis where applicable, and supportive next steps. Charts, mind map, PDFs, admin invite/resend, demo data, reset, pause/resume, and review/retake flows are supported. |
