...
async function performSearch() {
    ...
    try {
        // Search using the real RAG API (which already searches both blog and website)
        const apiResults = await searchKnowledgeBase(query);
        
        // Display results (don't fetch WordPress separately - API already does that)
        displayResults(apiResults, query);
    } catch (error) {
...
function formatMarkdownToHTML(text) {
    ...
    for (let line of lines) {
        ...
        // Unordered lists
            ...
            processed.push(`<li>${formatInline(content)}</li>`);
        }
        // Ordered lists
        ...
        // Regular paragraphs
            ...
            processed.push(`<p>${formatInline(trimmed)}</p>`);
        }
    }
...
function displayResults(results, query) {
    ...
    results.forEach((result, index) => {
        ...
        // Format the answer (convert markdown to HTML for AI-generated answers)
        const formattedAnswer = result.type === 'ai-generated' 
        ...
        resultCard.innerHTML = `
            <div class="result-header">
                ...
                ${result.type === 'ai-generated' ? 
                    '<span class="confidence-score" style="color: #10b981;" title="Generated by Google Gemini AI">✓ Verified Sources</span>' :
