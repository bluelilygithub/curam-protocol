...
function readDraft() {
  ...
  try { return JSON.parse(window.localStorage.getItem(CERQ_DRAFT_KEY) || 'null'); } catch { return null; }
}

function writeDraft(draft) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(CERQ_DRAFT_KEY, JSON.stringify(draft));
}

function clearDraft() {
  if (typeof window === 'undefined') return;
  window.localStorage.removeItem(CERQ_DRAFT_KEY);
}
...
function CerqResultPanel({ attempt, onDownloadPdf, pdfLoading }) {
  const scaleScores = parseMaybeJson(attempt?.scaleScores, []);
  const analysis = parseMaybeJson(attempt?.analysis, {});
  const strongest = analysis?.rationale?.strongest || [...scaleScores].sort((a, b) => Number(b.normalized) - Number(a.normalized)).slice(0, 4);
    ...
    <div className="space-y-4">
      <section className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h2 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>CERQ-style cognitive coping profile</h2>
      ...
      <section className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
        <h2 className="text-sm font-semibold mb-3" style={{ color: 'var(--color-text)' }}>Most frequent strategies</h2>
...
export default function CerqStylePanel({ onBack, onComplete, onNext, nextLabel = 'Continue' }) {
  ...
  const refreshAttempts = useCallback(async () => {
    const res = await api.get('/api/wellbeing/cerq/attempts');
    ...
    (async () => {
      ...
      try {
        const [questionRes] = await Promise.all([
          api.get('/api/wellbeing/cerq/questions'),
          refreshAttempts(),
        ]);
        const questionData = await questionRes.json();
        if (!questionRes.ok) throw new Error(questionData.error || 'Could not load CERQ-style check');
        setConfig(questionData);
        refreshDraftMeta();
      } catch (err) {
        setError(err.message || 'Could not load CERQ-style check');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const startAttempt = () => {
    if (draftMeta && !window.confirm('Start over and discard the paused CERQ-style draft?')) return;
    clearDraft();
    setDraftMeta(null);
    setMode('taking');
    setIndex(0);
  ...
  const submitCurrent = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Could not save CERQ-style attempt');
      setResult(data);
      setMode('result');
      clearDraft();
      setDraftMeta(null);
  ...
  const openAttempt = async (id) => {
    setError('');
    try {
      const res = await api.get(`/api/wellbeing/cerq/attempts/${id}`);
      const data = await res.json();
  ...
  const deleteAttempt = async (id) => {
    if (!window.confirm('Delete this completed CERQ-style profile?')) return;
    await api.delete(`/api/wellbeing/cerq/attempts/${id}`).catch(() => {});
    if (detail?.id === id) {
      setDetail(null);
      setMode('intro');
    }
  ...
  const downloadAttemptPdf = async (attempt) => {
    ...
    setError('');
    try {
      const res = await api.get(`/api/wellbeing/cerq/attempts/${attempt.id}/pdf`);
      if (!res.ok) throw new Error('PDF generation failed');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `cerq-style-${attempt.id}.pdf`;
  ...
  if (loading) return <div className="p-6 text-sm" style={{ color: 'var(--color-muted)' }}>Loading CERQ-style check...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-semibold" style={{ color: 'var(--color-text)' }}>CERQ-Style Cognitive Coping Check</h2>
          <p className="text-sm mt-1 max-w-3xl" style={{ color: 'var(--color-muted)' }}>
            A 36-item proof-of-concept profile of cognitive strategies after stress. It uses original wording and is not the official CERQ.
          </p>
        </div>
        <button type="button" onClick={startAttempt} className="px-4 py-2 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity" style={{ background: 'var(--color-primary)' }}>
          {draftMeta ? 'Start over' : 'Start CERQ-style check'}
        </button>
      </div>
      ...
      {mode === 'intro' && (
        <div className="grid lg:grid-cols-[1fr_360px] gap-6">
          ...
          <aside className="space-y-3">
            {draftMeta && (
              <div className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
                <p className="text-xs uppercase tracking-wider" style={{ color: 'var(--color-muted)' }}>Paused CERQ-style check</p>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <button
                type="button"
                onClick={() => {
                  if (window.confirm('Cancel this CERQ-style attempt and discard the paused draft?')) {
                    clearDraft();
                    setDraftMeta(null);
                    setMode('intro');
                  }
      ...
      {mode === 'detail' && detail && (
        <section className="max-w-4xl mx-auto space-y-4">
          ...
          <CerqResultPanel attempt={detail} onDownloadPdf={downloadAttemptPdf} pdfLoading={pdfLoadingId === detail.id} />
        </section>
      )}
      ...
      {mode === 'intro' && (
        <section className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--color-border)' }}>
          <div className="px-4 py-3 border-b" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <h3 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Past completed CERQ-style profiles</h3>
          </div>
          {attempts.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--color-muted)' }}>No completed CERQ-style profiles yet.</p>
          ) : (
                ...
                return (
                  <div key={attempt.id} className="flex items-center justify-between gap-3 px-4 py-3" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)' }}>
                    <button type="button" onClick={() => openAttempt(attempt.id)} className="flex-1 text-left hover:opacity-70 transition-opacity">
                      <p className="text-sm font-medium" style={{ color: 'var(--color-text)' }}>{highest ? `${highest.label} - ${highest.band}` : 'CERQ-style profile'}</p>
