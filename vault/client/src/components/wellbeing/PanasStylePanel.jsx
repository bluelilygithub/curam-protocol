...
function readDraft() {
  ...
  try { return JSON.parse(window.localStorage.getItem(PANAS_DRAFT_KEY) || 'null'); } catch { return null; }
}

function writeDraft(draft) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(PANAS_DRAFT_KEY, JSON.stringify(draft));
}

function clearDraft() {
  if (typeof window === 'undefined') return;
  window.localStorage.removeItem(PANAS_DRAFT_KEY);
}
...
function AffectScoreCard({ scale }) {
  const pct = Math.round(Number(scale.normalized || 0) * 100);
  const color = scale.family === 'negative' ? '#ef4444' : '#22c55e';
  return (
    <div className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="text-sm font-bold tabular-nums" style={{ color }}>{scale.score}/{scale.max}</span>
      </div>
      <div className="h-2 rounded-full overflow-hidden mt-3" style={{ background: 'var(--color-bg)' }}>
        <div className="h-full" style={{ width: `${pct}%`, background: color }} />
      </div>
      <p className="text-xs mt-2" style={{ color: 'var(--color-muted)' }}>{scale.band} current endorsement</p>
    </div>
  );
}

function PanasResultPanel({ attempt, onDownloadPdf, pdfLoading }) {
    ...
    <div className="space-y-4">
      <section className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h2 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>PANAS-style affect result</h2>
      ...
      <StrategyBarChart scales={scaleScores} responseMax={5} variant="panas" title="PANAS-style affect profile" />

      <div className="grid md:grid-cols-2 gap-3">
        {scaleScores.map((scale) => <AffectScoreCard key={scale.key} scale={scale} />)}
      </div>
...
export default function PanasStylePanel({ onBack, onComplete, onNext, nextLabel = 'Continue' }) {
  ...
  const refreshAttempts = useCallback(async () => {
    const res = await api.get('/api/wellbeing/panas/attempts');
    ...
    (async () => {
      ...
      try {
        const [questionRes] = await Promise.all([
          api.get('/api/wellbeing/panas/questions'),
          refreshAttempts(),
        ]);
        const questionData = await questionRes.json();
        if (!questionRes.ok) throw new Error(questionData.error || 'Could not load PANAS-style check');
        setConfig(questionData);
        refreshDraftMeta();
      } catch (err) {
        setError(err.message || 'Could not load PANAS-style check');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const startAttempt = () => {
    if (draftMeta && !window.confirm('Start over and discard the paused PANAS-style draft?')) return;
    clearDraft();
    setDraftMeta(null);
    setMode('taking');
    setIndex(0);
  ...
  const submitCurrent = async () => {
    ...
    setError('');
    startProcessing(
      'Saving your PANAS-style affect check...',
      'We are scoring positive and negative affect, then preparing the considered response. This can take a little while, so please stay on this screen until it finishes.'
    );
    try {
      const res = await api.post('/api/wellbeing/panas/attempts', { answers: nextAnswers });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Could not save PANAS-style attempt');
      setResult(data);
      setMode('result');
      clearDraft();
      setDraftMeta(null);
      ...
      setError(err.message || 'Could not save PANAS-style attempt');
    } finally {
      stopProcessing();
      setSubmitting(false);
    }
  ...
  const openAttempt = async (id) => {
    setError('');
    try {
      const res = await api.get(`/api/wellbeing/panas/attempts/${id}`);
      const data = await res.json();
  ...
  const deleteAttempt = async (id) => {
    if (!window.confirm('Delete this completed PANAS-style check?')) return;
    await api.delete(`/api/wellbeing/panas/attempts/${id}`).catch(() => {});
    if (detail?.id === id) {
      setDetail(null);
      setMode('intro');
    }
  ...
  const downloadAttemptPdf = async (attempt) => {
    ...
    setError('');
    try {
      const res = await api.get(`/api/wellbeing/panas/attempts/${attempt.id}/pdf`);
      if (!res.ok) throw new Error('PDF generation failed');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `panas-style-${attempt.id}.pdf`;
  ...
  if (loading) return <div className="p-6 text-sm" style={{ color: 'var(--color-muted)' }}>Loading PANAS-style check...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-semibold" style={{ color: 'var(--color-text)' }}>PANAS-Style Affect Check</h2>
          <p className="text-sm mt-1 max-w-3xl" style={{ color: 'var(--color-muted)' }}>
            A 20-item proof-of-concept snapshot of current positive and negative affect. It is not the official PANAS and is not professional advice.
          </p>
        </div>
        <button type="button" onClick={startAttempt} className="px-4 py-2 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity" style={{ background: 'var(--color-primary)' }}>
          {draftMeta ? 'Start over' : 'Start PANAS-style check'}
        </button>
      </div>
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-6">
          <div className="space-y-4">
            <QuizPurposePanel
              ...
              summary="This check captures current positive and negative affect so the combined report can distinguish mood severity from immediate emotional tone."
              points={[
                'It scores positive affect and negative affect separately.',
                'It helps identify mixed emotional states, such as high distress with some remaining energy or low activation without high distress.',
                'It adds a state-based affect lens to the broader mood, personality, and coping profile.',
              ]}
              guidance={[
                'Answer for how you feel recently or right now, according to the instructions on the page.',
                'Do not try to make the positive and negative answers match; both can be high, low, or mixed.',
                'Treat it as a snapshot rather than a fixed description of who you are.',
              ]}
              caveat="This is a proof-of-concept PANAS-style tool using original wording. It is not the official PANAS, not a diagnosis, and not a substitute for professional advice."
            />
            <section className="rounded-2xl border p-5 space-y-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
              ...
              <p className="text-sm" style={{ color: 'var(--color-muted)' }}>
                Rate how strongly each affect statement describes your recent emotional state. The two scores are interpreted separately, not collapsed into a single wellbeing score.
              </p>
              <ul className="text-sm space-y-2" style={{ color: 'var(--color-muted)' }}>
                <li>20 statements are answered in order.</li>
                <li>Responses use a five-point intensity scale.</li>
            ...
            <div className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
              ...
              {latestScales.length > 0 && (
                <p className="text-xs mt-1" style={{ color: 'var(--color-muted)' }}>
                  Latest: {latestScales.map((scale) => `${scale.label} ${scale.score}/${scale.max}`).join(' · ')}
                </p>
              )}
            </div>
          </aside>
        ...
        <section className="max-w-2xl mx-auto space-y-4">
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <button type="button" onClick={() => { if (window.confirm('Cancel this PANAS-style attempt and discard the paused draft?')) { clearDraft(); setDraftMeta(null); setMode('intro'); } }} className="px-3 py-2 rounded-xl text-sm border hover:opacity-70 transition-opacity" style={{ borderColor: '#fecaca', color: '#991b1b' }}>
                Cancel
              </button>
      ...
      {mode === 'result' && result && (
        <section className="max-w-4xl mx-auto space-y-4">
          ...
          <PanasResultPanel attempt={result} onDownloadPdf={downloadAttemptPdf} pdfLoading={pdfLoadingId === result.id} />
        </section>
      )}
      ...
      {mode === 'detail' && detail && (
        <section className="max-w-4xl mx-auto space-y-4">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={() => setMode('intro')} className="text-sm hover:opacity-70" style={{ color: 'var(--color-primary)' }}>Back to PANAS overview</button>
            <button type="button" onClick={() => deleteAttempt(detail.id)} className="text-sm hover:opacity-70" style={{ color: '#dc2626' }}>Delete</button>
          </div>
          <PanasResultPanel attempt={detail} onDownloadPdf={downloadAttemptPdf} pdfLoading={pdfLoadingId === detail.id} />
        </section>
      )}
      ...
      {mode === 'intro' && (
        <section className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--color-border)' }}>
          <div className="px-4 py-3 border-b" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <h3 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Past completed PANAS checks</h3>
          </div>
          {attempts.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--color-muted)' }}>No completed PANAS checks yet.</p>
          ) : (
            <div className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
              {attempts.map((attempt) => {
                const scales = parseMaybeJson(attempt.scaleScores, []);
                return (
                  <div key={attempt.id} className="flex items-center justify-between gap-3 px-4 py-3" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)' }}>
                    <button type="button" onClick={() => openAttempt(attempt.id)} className="flex-1 text-left hover:opacity-70 transition-opacity">
                      <p className="text-sm font-medium" style={{ color: 'var(--color-text)' }}>{scales.map((scale) => `${scale.label}: ${scale.score}/${scale.max}`).join(' · ') || 'PANAS-style affect check'}</p>
