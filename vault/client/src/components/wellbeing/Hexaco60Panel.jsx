...
function readDraft() {
  ...
  try { return JSON.parse(window.localStorage.getItem(HEXACO_DRAFT_KEY) || 'null'); } catch { return null; }
}

function writeDraft(draft) {
  if (typeof window === 'undefined') return;
  window.localStorage.setItem(HEXACO_DRAFT_KEY, JSON.stringify(draft));
}

function clearDraft() {
  if (typeof window === 'undefined') return;
  window.localStorage.removeItem(HEXACO_DRAFT_KEY);
}
...
function HexacoResultPanel({ attempt, onDownloadPdf, pdfLoading }) {
    ...
    <div className="space-y-4">
      <section className="rounded-2xl border p-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <h2 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>HEXACO-60-style profile</h2>
      ...
      <DomainRadarChart domains={domainScores} title="HEXACO six-domain profile" />

      <div className="grid md:grid-cols-2 gap-3">
        {domainScores.map((domain) => <DomainCard key={domain.key} domain={domain} />)}
      </div>
...
export default function Hexaco60Panel({ onBack, onComplete, onNext, nextLabel = 'Continue' }) {
  ...
  const refreshAttempts = useCallback(async () => {
    const res = await api.get('/api/wellbeing/hexaco/attempts');
    ...
    (async () => {
      ...
      try {
        const [questionRes] = await Promise.all([
          api.get('/api/wellbeing/hexaco/questions'),
          refreshAttempts(),
        ]);
        const questionData = await questionRes.json();
        if (!questionRes.ok) throw new Error(questionData.error || 'Could not load HEXACO-60-style check');
        setConfig(questionData);
        refreshDraftMeta();
      } catch (err) {
        setError(err.message || 'Could not load HEXACO-60-style check');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const startAttempt = () => {
    if (draftMeta && !window.confirm('Start over and discard the paused HEXACO-60-style draft?')) return;
    clearDraft();
    setDraftMeta(null);
    setMode('taking');
    setIndex(0);
  ...
  const submitCurrent = async () => {
    ...
    setError('');
    startProcessing(
      'Saving your HEXACO-60-style profile...',
      'We are scoring the six personality domains and preparing the considered response. This can take a little while, so please stay on this screen until it finishes.'
    );
    try {
      const res = await api.post('/api/wellbeing/hexaco/attempts', { answers: nextAnswers });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Could not save HEXACO-60-style attempt');
      setResult(data);
      setMode('result');
      clearDraft();
      setDraftMeta(null);
      ...
      setError(err.message || 'Could not save HEXACO-60-style attempt');
    } finally {
      stopProcessing();
      setSubmitting(false);
    }
  ...
  const openAttempt = async (id) => {
    setError('');
    try {
      const res = await api.get(`/api/wellbeing/hexaco/attempts/${id}`);
      const data = await res.json();
  ...
  const deleteAttempt = async (id) => {
    if (!window.confirm('Delete this completed HEXACO-60-style profile?')) return;
    await api.delete(`/api/wellbeing/hexaco/attempts/${id}`).catch(() => {});
    if (detail?.id === id) {
      setDetail(null);
      setMode('intro');
    }
  ...
  const downloadAttemptPdf = async (attempt) => {
    ...
    setError('');
    try {
      const res = await api.get(`/api/wellbeing/hexaco/attempts/${attempt.id}/pdf`);
      if (!res.ok) throw new Error('PDF generation failed');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `hexaco-60-style-${attempt.id}.pdf`;
  ...
  if (loading) return <div className="p-6 text-sm" style={{ color: 'var(--color-muted)' }}>Loading HEXACO-60-style check...</div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-semibold" style={{ color: 'var(--color-text)' }}>HEXACO-60-Style Personality Check</h2>
          <p className="text-sm mt-1 max-w-3xl" style={{ color: 'var(--color-muted)' }}>
            A 60-item proof-of-concept personality self-report across six HEXACO-style domains. This is not the official HEXACO-PI-R and is not professional advice.
          </p>
        </div>
        <button type="button" onClick={startAttempt} className="px-4 py-2 rounded-xl text-sm font-semibold text-white hover:opacity-90 transition-opacity" style={{ background: 'var(--color-primary)' }}>
          {draftMeta ? 'Start over' : 'Start HEXACO-60-style check'}
        </button>
      </div>
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-6">
          <div className="space-y-4">
            <QuizPurposePanel
              ...
              summary="This check adds a six-domain personality lens to the combined wellbeing profile. It is especially useful because Honesty-Humility and Emotionality can add nuance that is not captured by the five-domain IPIP-style profile alone."
              points={[
                'It estimates relative endorsement across six HEXACO-style domains.',
                'It adds a second personality lens to compare with IPIP-NEO-120 results.',
                'It can help the combined profile distinguish interpersonal style, emotional sensitivity, conscientiousness, and openness patterns.',
              ]}
              guidance={[
                'Answer based on how you generally tend to be, not only how you feel today.',
                'Respond to the wording directly; some items are reverse-scored automatically.',
                'Use it as reflection data, not as a label or assessment of worth, ability, or character.',
              ]}
              caveat="This is a proof-of-concept HEXACO-style tool using original item wording. It is not the official HEXACO-PI-R, not a diagnosis, and not a substitute for professional advice."
            />
            <section className="rounded-2xl border p-5 space-y-4" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
              <div>
                ...
                <p className="text-sm" style={{ color: 'var(--color-muted)' }}>
                  Answer each statement according to your usual tendencies. The result estimates broad self-report patterns, not diagnoses, suitability, capability, or professional advice.
                </p>
              </div>
              <ul className="text-sm space-y-2" style={{ color: 'var(--color-muted)' }}>
                <li>60 statements are answered in order.</li>
                <li>Responses use a five-point agreement scale.</li>
      ...
      {mode === 'taking' && current && (
        <section className="max-w-2xl mx-auto space-y-4">
          ...
          <div className="rounded-2xl border p-5" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <p className="text-xs uppercase tracking-wider mb-1" style={{ color: 'var(--color-muted)' }}>{current.domainLabel}</p>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap justify-end gap-2">
              <button type="button" onClick={() => { if (window.confirm('Cancel this HEXACO-60-style attempt and discard the paused draft?')) { clearDraft(); setDraftMeta(null); setMode('intro'); } }} className="px-3 py-2 rounded-xl text-sm border hover:opacity-70 transition-opacity" style={{ borderColor: '#fecaca', color: '#991b1b' }}>
                Cancel
              </button>
      ...
      {mode === 'result' && result && (
        <section className="max-w-4xl mx-auto space-y-4">
          ...
          <HexacoResultPanel attempt={result} onDownloadPdf={downloadAttemptPdf} pdfLoading={pdfLoadingId === result.id} />
        </section>
      )}
      ...
      {mode === 'detail' && detail && (
        <section className="max-w-4xl mx-auto space-y-4">
          <div className="flex items-center justify-between gap-3">
            <button type="button" onClick={() => setMode('intro')} className="text-sm hover:opacity-70" style={{ color: 'var(--color-primary)' }}>Back to HEXACO overview</button>
            <button type="button" onClick={() => deleteAttempt(detail.id)} className="text-sm hover:opacity-70" style={{ color: '#dc2626' }}>Delete</button>
          </div>
          <HexacoResultPanel attempt={detail} onDownloadPdf={downloadAttemptPdf} pdfLoading={pdfLoadingId === detail.id} />
        </section>
      )}
      ...
      {mode === 'intro' && (
        <section className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--color-border)' }}>
          <div className="px-4 py-3 border-b" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
            <h3 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Past completed HEXACO profiles</h3>
          </div>
          {attempts.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: 'var(--color-muted)' }}>No completed HEXACO profiles yet.</p>
          ) : (
                ...
                return (
                  <div key={attempt.id} className="flex items-center justify-between gap-3 px-4 py-3" style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)' }}>
                    <button type="button" onClick={() => openAttempt(attempt.id)} className="flex-1 text-left hover:opacity-70 transition-opacity">
                      <p className="text-sm font-medium" style={{ color: 'var(--color-text)' }}>{highest ? `${highest.label} - ${highest.band}` : 'HEXACO-60-style profile'}</p>
