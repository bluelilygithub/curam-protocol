...
function TopicCard({ result, date, onCommentarySave }) {
  ...
  // Rendering keys off what THIS DAY's analysis actually contains, not the topic's current
  // template setting — template is editable any time (see PUT /topics/:id), and a switch must
  // never break already-cached days. A "digest" day has keyStorylines and no left/right; a
  // "perspectives" day (including every pre-template-feature legacy row) has left/right/
  // commonGround and no keyStorylines.
  const isDigestShape = !!(analysis || {}).keyStorylines;
    ...
    >
      ...
      {expanded && (
        <div className="px-5 pb-5 space-y-4">
          ...
          {/* Shape is per-day, derived from what this analysis actually contains — see
              isDigestShape above. A "digest"-shaped day gets a single Key Storylines block;
              everything else (including legacy pre-template rows) gets the full 4-section
              spread. */}
          {isDigestShape ? (
...
function TopicForm({ initial, availableGroups, onSave, onCancel }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <p className="text-xs mb-1.5" style={{ color: 'var(--color-muted)' }}>
          Format — editable any time. Switching only affects days generated from now on; days
          already saved keep rendering however they were originally generated.
        </p>
      ...
      <div className="flex gap-2 justify-end">
        ...
        <button
          onClick={() => title.trim() && onSave({ title: title.trim(), keywords: keywords.trim(), sourceGroups, template })}
          disabled={!title.trim()}
...
function TopicsPanel({ topics, onTopicsChange }) {
  ...
  const handleEdit = async (id, { title, keywords, sourceGroups, template }) => {
    try {
      const res = await api.put(`/api/news-digest/topics/${id}`, { title, keywords, sourceGroups, template });
          ...
          {editingId === topic.id ? (
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs mt-0.5" style={{ color: 'var(--color-muted)' }}>
                  Format: {TEMPLATE_OPTIONS.find(o => o.value === topic.template)?.label || 'Perspectives'}
                </p>
              </div>
