...
- `server/routes/clients.js` (CRM, flag `clients`) — canonical client table (`clients`), shared by Projects, Tasks, and Finance (`server/routes/finance.js` "Clients"/invoices sections read/write it via `clientRef` + the `client_billing_details` extension table for abn/address). `fin_clients` retired and dropped 2026-09-16 — see **`docs/crm-migration.md`** for the completed migration history and rollback notes.
