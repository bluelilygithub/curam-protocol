...
**Finance** (`/finance`): Invoices + Quotes tabs (`fin_invoices`/`fin_invoice_items`, `docType` distinguishes). Row actions: Send/Resend (opens send modal, PDF attached via `invoicePdf.js`), Edit, Convert (quote→invoice), Paid, Del, PDF download, **Activity** (opens the same detail modal used by clicking a row — shows `fin_invoice_send_log` history: sent-to, cc, provider, messageId, pdfAttached, ok/error). Shared `POST /invoices/:id/send` route (`server/routes/finance.js`) always CCs the workspace `fin_admin_email` setting (Settings → Finance) on every send/resend, invoice or quote — logged in `fin_invoice_send_log.cc`.
