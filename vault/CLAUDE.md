...
- `client/src/pages/FontsPage.jsx` + `client/src/pages/fonts/` — Font Customizer Phase 2: browser-only live preview (`/fonts`, flag `fonts`). Loads a `.ttf/.otf/.woff` via `opentype.js` client-side — no backend calls during interactive use (export, wiring to the Phase 1 Python backend, is a later phase). Proofing text presets (`proofingPresets.js`), designer-labeled parametric transforms applied to the opentype.js path per-frame as visual approximations only (`fontGlyphTransforms.js` — Stem Thickness, Proportional Width, Extend Ascenders/Descenders, Counter Width), class-based kerning groups + advanced pair overrides (`fontKerningClasses.js`, `FontKerningPanel.jsx`), and "Parametric Stylesheet" presets stored as the transform recipe (not a baked result) in `localStorage` (`fontPresetsStorage.js`).
