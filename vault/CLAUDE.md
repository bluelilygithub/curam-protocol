...
- `server/services/fonts/` — Google Font Customizer backend, Python module — see `server/services/fonts/README.md`. Wired to Node via `server/services/fontExportPipeline.js` (`runFontExport()`, subprocess pattern matching `officeConvert.js`/`videoFfmpeg.js`) + `server/routes/fonts.js` (`POST /api/fonts/customize-export`, flag `fonts`) → `server/services/fonts/cli_export.py` (`python -m fonts.cli_export`, cwd `server/services/`). Font picker/search: `server/services/fontGoogleCatalog.js` caches an OFL-filtered Google Fonts catalog (`GET /api/fonts/catalog`) built by `cli_catalog.py` via `google_fonts_repo.list_ofl_family_slugs()` — the same directory-presence rule `find_family()` uses, not a second license check; picking a result still runs the real Phase 1 fetch/freeze/license-check at export time, unchanged. Phase 1: fetch from `google/fonts` GitHub repo + OFL license check + fontTools variable/static inspection + freeze-to-static (`pipeline.py`). Phase 3/3.5: real fontTools structural edits (`structural.py`, `transforms/`) — stem thickness via outline offset, counter width via winding-direction contour classification, proportional width, ascender/descender extension, sidebearing recalc, contour validation, GPOS class-kerning (`kerning.py`) — composite/accented glyphs (á, é, ñ...) fully decomposed and reassembled, not skipped. Phase 4: export (`export/`) — OFL Reserved Font Name rename (blocks export if unchanged), table cleanup, subsetting (`fontTools.subset`, glyph selection never filtered by `numberOfContours`), genuine glyf→CFF `.otf` conversion alongside `.ttf`/`.woff2`, one `ExportReport` reusing Phase 3.5's coverage report. Architecture: structural edits (kerning, letterform shape) belong in the font binary via fontTools; color/shadow/image-fill stay in the CSS/SVG rendering layer, never baked into color-font formats (COLRv1/sbix/CBDT deliberately unsupported).
- `client/src/pages/FontsPage.jsx` + `client/src/pages/fonts/` — Font Customizer frontend (`/fonts`, flag `fonts`), two modes via a top switch. Guided tour (`client/src/utils/tours/fontsTour.js`, compass icon in the header, same Shepherd.js pattern as `recipesTour.js`/`financeTour.js`) walks both modes end to end since it can't `navigate()` between them (same page, internal state) — steps call `setPageMode`/`setActiveTab` then wait a beat before attaching, via a `switchTo()` helper. Tooltips (`Tooltip` from `client/src/components/Tooltip.jsx`, same convention as Graphics) cover every meaningful control across both modes, not just the Phase 2 transform sliders. **Structural Preview** (Phase 2): loads a `.ttf/.otf/.woff` via `opentype.js` client-side, no backend calls — proofing text presets (`proofingPresets.js`), designer-labeled parametric transforms as visual approximations only (`fontGlyphTransforms.js`), class-based kerning groups + advanced overrides (`fontKerningClasses.js`, `FontKerningPanel.jsx`), "Parametric Stylesheet" presets in `localStorage` (`fontPresetsStorage.js`). **Effects & Export** (Phase 5, `FontEffectsPanel.jsx`): loads the *actual* Phase 4 export via a real `FontFace`/`@font-face` — never the Phase 2 canvas approximation — either automatically (the Export tab's `FontExportTrigger.jsx` posts the transform/kerning recipe + OFL rename fields to `POST /api/fonts/customize-export`, which runs the Python pipeline server-side and hands the result straight to the preview) or via manual drag-in of a `.woff2`/`.ttf`/`.otf` + coverage-report JSON (kept as a fallback/advanced path for anyone running the Python pipeline independently). **`opentype.js` cannot decode `.woff2`** (no Brotli decompressor built in, and the `wawoff2` wasm package isn't actually browser-safe — its Emscripten build assumes Node's `fs`, confirmed by Vite silently externalizing `fs`/`path` rather than erroring) — the automatic path picks `.ttf`/`.otf` from the export response for in-app parsing (still a real Phase 4 file, just a format this parser can read); a manually dropped `.woff2` gets a clear message steering to `.ttf`/`.otf` instead of opentype.js's raw `"Unsupported OpenType signature wOF2"` error. `.woff2` itself is still the right file to host in production — browsers decode it natively — this limitation is in-app preview parsing only. Reads the renamed family name straight from the loaded file's own name table, cross-checks preview text against the coverage report (`coverageCheck.js`, matches by **Unicode codepoint**, never glyph name — many production webfonts, including Google Fonts' own Roboto, ship a `post` table format with no glyph names baked in at all, which silently broke a name-based check for every glyph; existence uses `glyph.index === 0`, skipped-at-export uses `glyphs_skipped_chars` from `export/pipeline.py`'s `ExportReport.summary()`, which enriches Phase 3.5's name-keyed skip list with the font's own cmap-derived characters), and generates a copyable CSS snippet (`effectsCss.js`) for solid/gradient/image-fill (`background-clip: text`) plus layered `text-shadow` — rendering-layer only, never touches the font binary. **Nothing in this tool persists server-side** (no DB table, no library) — the "Download your font" row keeps every format the export produced as in-memory `ArrayBuffer`s (`downloadBuffers` state) so the designer can actually save the `.ttf`/`.woff2`/`.otf` files, not just copy CSS or export an SVG; closing the tab without downloading loses them. **Print handoff** (`svgExport.js`): "Export Outlined SVG" flattens glyph outlines to `<path>` elements (never `<text>`) from the same parsed `opentype.js` `Font` object already used for the live preview/coverage check — never a second independent parse — bakes the active fill as a `<linearGradient>`/`<pattern>`, shadows as blurred offset duplicate paths (`<feGaussianBlur>`) behind the main glyph path, and is blocked (button disabled) whenever the coverage check has an unresolved character. Explicitly complementary to the real font file in the UI copy — not a substitute for installing it where editable text is still needed (e.g. InDesign pre-flatten).
...
Projects · Folders · Chat (project + general) · Files (RAG) · Personas · Prompts · Memory · **Suggestions inbox** · Pinned URLs · Document Compare · **Document redaction** · Multi-Model Debate · Tasks (list/board/calendar/matrix) · Goals (OKR-lite) · Chat History · Web Search (`@search`, Brave/Serper/SerpAPI) · Gmail integration · Google Calendar · Google Drive backup · News Digest · Finance · Admin dashboard + user management · Password reset · Shared task public links · **Student** (Quiz + Cards + Saved decks) · **Shares** (portfolio tracker) · **Product Scout** (Amazon comparison agent) · **Video Tools** (ffmpeg + FAL generate) · **YouTube** (search + favourites + history, no download — see `docs/youtube-agent.md`) · **Recipes** (leftover cooking assistant) · **Google Ads** (site scrape + keywords / RSA) · **SEO** (crawl + per-page recommendations) · **Search** (Google Search Console queries/pages) · **HTML** (Lighthouse / PageSpeed) · **Property Scenario** (mortgage / property calcs + CDR + document insights) · **PDF Tools** (merge/split/rotate/watermark, AcroForm field designer, Office↔PDF, Drive↔PDF) · **Font Customizer** (OFL Google Fonts, variable-font freeze via fontTools backend + browser-only opentype.js live preview, phase 1-2 of a multi-phase build — export/binary editing not yet wired)
