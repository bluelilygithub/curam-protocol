...
- `server/routes/clients.js` (CRM, flag `clients`) + `server/routes/finance.js` "Clients" section — **being merged**, `clients` is canonical (Projects/Tasks already key off it), `fin_clients` retiring. In-progress migration + rollback plan: **`docs/crm-migration.md`**. Bridge column `fin_clients."clientId"` + extension table `client_billing_details` (abn/address) already in `server/db.js`; backfill script `server/scripts/migrateFinClientsToClients.js` not yet run against production.
