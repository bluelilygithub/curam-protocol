...
- `server/routes/clients.js` (displayed as **"CRM"** in nav/UI, internal key/route/files still say "clients", flag `clients`) — canonical client table (`clients`) + contacts, deals/pipeline (`server/routes/deals.js`), touchpoints, Gmail search, on-demand AI activity summary (`POST /:id/summary`, `light` tier). Shared by Projects, Tasks, and Finance (`server/routes/finance.js` invoices/quotes read it via `clientRef` + the `client_billing_details` extension table for abn/address — Finance's own client-management UI/routes were removed, it's picker-only now). `fin_clients` retired and dropped 2026-09-16 — see **`docs/crm-migration.md`** (clients/fin_clients merge) and **`docs/crm-deals-schema.md`** (deals, communication-history channel filter, attachments, calendar export) for history and rollback notes. Touchpoints: filterable by `type`/channel (reused, no new column), "+ Follow up" bridges a touchpoint to a linked Task (one direction only — touchpoints stay a pure past-tense log), and support file attachments. `ClientDetailPage.jsx`'s **Tasks** section shows every open task linked to the client, whether via a project or directly (`tasks.clientId`, e.g. a follow-up) — `ProjectsSection`'s own task counts only match `projectId` and won't show a directly-linked one. Client page also has a Shepherd tour (`utils/tours/crmTour.js`, compass icon) and an info modal (`components/CrmInfoModal.jsx`, info icon, auto-shows once) explaining the touchpoint/follow-up/task model.
- `server/utils/attachments.js` — shared attachment policy (upload allowlist, 50MB cap, per-user quota, disguised-executable magic-byte check) used by both touchpoints (`clients.js`) and tasks (`tasks.js`); extracted once a second entity type needed the same logic. `server/routes/attachments.js` — generic cross-entity `GET/DELETE /api/attachments/:id` (download/delete); upload stays per-entity since destination + ownership checks differ. `client/src/components/AttachmentChip.jsx` — shared display (image thumbnail via blob fetch, filename chip otherwise).
...
- `server/services/browserAgent/` — **Browser Agent** backend: `browserAgentSession.js` (`BrowserAgentSession`, one Playwright `BrowserContext` per WS connection, Anthropic tool-use loop, the "never submits" guard — submit-click block + network-level POST block while the agent has control), `browserAgentWs.js` (WS upgrade at `/api/browser-agent/ws`, hand-rolled `requireAuth`-equivalent since the upgrade handshake skips Express middleware). Route `server/routes/browserAgent.js` (run archive + saved-login CRUD). Conversation memory (`this.messages`) persists for the WS session, not per instruction. Saved site logins (`browser_agent_credentials`, encrypted at rest) fill via `fill_login` server-side only — the password never reaches the model. Client: `client/src/pages/BrowserAgentPage.jsx` (`/browser-agent`, live stream + instruction box + presets + pause/resume/take-over/clear-session), `BrowserAgentSettingsPage.jsx` (`/browser-agent/settings`), `BrowserAgentArchivePage.jsx` (`/browser-agent/archive`). Feature flag `browserAgent`. Docs: **`docs/browser-agent.md`**.
...
Projects · Folders · Chat (project + general) · Files (RAG) · Personas · Prompts · Memory · **Suggestions inbox** · Pinned URLs · Document Compare · **Document redaction** · Multi-Model Debate · Tasks (list/board/calendar/matrix, file attachments, one-way `.ics` calendar export) · Goals (OKR-lite) · Chat History · Web Search (`@search`, Brave/Serper/SerpAPI) · Gmail integration · Google Calendar · Google Drive backup · News Digest · Finance · **CRM** (contacts, deals/pipeline, touchpoints with channel filter + file attachments + task follow-up bridge, Gmail search — internal key "clients") · Admin dashboard + user management · Password reset · Shared task public links · **Student** (Quiz + Cards + Saved decks) · **Shares** (portfolio tracker) · **Product Scout** (Amazon comparison agent) · **Video Tools** (ffmpeg + FAL generate) · **YouTube** (search + favourites + history, no download — see `docs/youtube-agent.md`) · **Recipes** (leftover cooking assistant) · **Google Ads** (site scrape + keywords / RSA) · **SEO** (crawl + per-page recommendations) · **Search** (Google Search Console queries/pages) · **HTML** (Lighthouse / PageSpeed) · **Property Scenario** (mortgage / property calcs + CDR + document insights) · **PDF Tools** (merge/split/rotate/watermark, AcroForm field designer, Office↔PDF, Drive↔PDF) · **Font Customizer** (search an OFL Google Font, adjust real structural properties — stem thickness, width, ascender/descender, counter width, kerning — see the actual transformed font via debounced server-side preview, download a real renamed `.ttf`/`.woff2`/`.otf`; single screen, no client-side approximation) · **CSS** (non-technical CSS editor, internal key "restyle" — built-in demo page, upload/paste HTML + reorderable CSS, sandboxed live preview, click-to-edit property panel incl. text enhancements + animation presets, voice or plain-English AI requests, save pages to revisit later, PostCSS auto-fix/flags, no CSS jargon in any UI copy) · **Browser Agent** (live-streamed headless-Chromium form filling that never submits — see below)

**Browser Agent** (`/browser-agent`): Live-streamed headless Chromium (CDP screencast) driven by an Anthropic tool-use loop to fill forms on a named site — booking/enquiry forms, saved-login sign-ins — and stop. It never submits: submit/pay/sign-in buttons are blocked in code, and a Playwright network-route guard blocks any form POST while the agent has control; it always ends by handing the filled form to the user to review and send themselves. Profile (name/phone/email/address) and saved site logins (encrypted, filled server-side, password never reaches the model) live on their own Settings page. Conversation memory persists across follow-up instructions in a session; Clear session wipes it. Pause/Resume/Take over give three levels of interruption. Presets save reusable instructions; Archive keeps every past run with its full step log, outcome, duration and any error screenshots. Feature flag `browserAgent`. Docs: **`docs/browser-agent.md`**.
...
**Finance** (`/finance`): Real double-entry bookkeeping for a sole trader, not just Invoices/Quotes — every posting goes through a shared journal helper that throws on any debit/credit imbalance, so the books stay balanced by construction. Full tab list: Dashboard, Invoices, Quotes, Suppliers, Expenses, Drawings, Vehicle/Home Office, Assets, Recurring, Wages, Interest, Journal, Accounts, Codes, BAS, Position, Balances, Reports, Settings. Covers chart of accounts, wages vs owner's drawings, vehicle/home-office ATO deduction calculators (cents-per-km/logbook, fixed-rate/actual-cost, method locked per financial year), an assets register with real depreciation (immediate/low-value-pool/prime-cost/diminishing-value) including purchase journal + GST split + disposal, recurring entries, manual journal, cash-basis BAS/GST lifecycle (open→reconciled→lodged→paid), Trial Balance/P&L/Balance Sheet/GST Summary reports + charts, and MYOB/Xero/Excel/Google-Sheets export with a cutoff-date lock against duplicate imports. Route: `server/routes/finance.js` (large — see **`docs/finance.md`** for the accounting design, invariants, and known gaps before touching it). Client: `client/src/pages/FinancePage.jsx`.

Invoices/Quotes specifically (`fin_invoices`/`fin_invoice_items`, `docType` distinguishes): row actions Send/Resend (opens send modal, PDF attached via `invoicePdf.js`), Edit, Convert (quote→invoice), Paid, Del, PDF download, **Activity** (opens the same detail modal used by clicking a row — shows `fin_invoice_send_log` history: sent-to, cc, provider, messageId, pdfAttached, ok/error). Shared `POST /invoices/:id/send` route always CCs the workspace `fin_admin_email` setting (Settings → Finance) on every send/resend, invoice or quote — logged in `fin_invoice_send_log.cc`; two failed sends for the same document within 24h also raises a Suggestions-inbox alert.
...
| `ATTACHMENT_QUOTA_MB` | Per-user total storage cap for touchpoint/task attachments (default 500) — `server/utils/attachments.js` |
