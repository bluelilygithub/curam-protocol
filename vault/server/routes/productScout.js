...
router.get('/runs/:id/pdf', async (req, res) => {
  try {
    ...
    const buffer = await buildProductScoutReportPdfBuffer(run.result);
    const safeName = String(run.query || 'product-scout').trim().slice(0, 60).replace(/[^a-z0-9]+/gi, '-').toLowerCase() || 'product-scout';
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${safeName}-report.pdf"`);
    res.send(Buffer.from(buffer));
  } catch (err) {
    console.error('[productScout] pdf', err.message);
    res.status(500).json({ error: err.message || 'Failed to generate PDF' });
  }
});

router.post('/runs/:id/email', handle(async (req) => {
  ...
  await sendEmail({
    to,
    subject: `Product Scout report: ${run.query}`,
    html: `<p>Attached is your Product Scout report for "<strong>${run.query}</strong>".</p><p>Generated by Curam Vault.</p>`,
    attachments: [{ filename: `${safeName}-report.pdf`, content: Buffer.from(buffer), contentType: 'application/pdf' }],
  });
  return { ok: true, to };
}));
