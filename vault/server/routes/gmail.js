...
router.get('/callback', async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      ...
      ).run(userId, tokens.access_token, tokens.refresh_token || null, tokens.token_type || 'Bearer', tokens.expiry_date || null, tokens.scope || null, email);
    }
...
router.post('/disconnect', (req, res) => {
  ...
  if (row) {
    try {
      const oauth2Client = getOAuth2Client();
      oauth2Client.revokeToken(row.accessToken).catch(() => {});
    } catch (_) {}
