...
/**
 * Generate plausible synthetic replacements via the document-redaction local slot.
 * Model id MUST come from resolveDocumentRedactionModels — never hardcoded.
 */
...
- Do NOT use placeholders like [NAME], XXX, REDACTED, or black bars.
...
function heuristicFallback(realValue, categoryLabel, seed) {
  ...
  if (seed && seed !== real && !/^\[.*\]$|^REDACTED/i.test(seed)) return seed;
  ...
  if (/financial|money|amount|dollar|invoice|salary|price/.test(cat) || /^\$/.test(real.trim())) {
  ...
  // Last resort: lightly obfuscate while staying readable
...
/**
 * @param {object} opts
 * @param {string} opts.modelId — from document redaction local slot
 * @param {Array<{entityKey, realValue, categoryLabel, seedReplacement?}>} opts.entities
 * @returns {Promise<{ map: Map<string,string>, errors: string[], modelId: string }>}
 */
async function generateSyntheticReplacements({ modelId, entities }) {
  const list = entities || [];
  const map = new Map();
  const errors = [];

  if (!list.length) return { map, errors, modelId };

  // Honour explicit user seeds that already look like real synthetics — still ask LLM to fill gaps
  const needLlm = [];
  for (const e of list) {
    const seed = e.seedReplacement ? String(e.seedReplacement).trim() : '';
    const userLocked = Boolean(e.userLocked);
    if (userLocked && seed && seed !== e.realValue) {
  ...
  if (needLlm.length && modelId) {
    try {
      ...
      for (const row of rows) {
        ...
        if (!key || !syn) continue;
        const ent = needLlm.find((e) => e.entityKey === key);
  ...
  for (const e of list) {
    ...
    map.set(
      e.entityKey,
      heuristicFallback(e.realValue, e.categoryLabel, e.seedReplacement),
    );
  }

  return { map, errors, modelId };
}
