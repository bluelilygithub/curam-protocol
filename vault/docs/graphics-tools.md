...
- **Export for Social** — one image in, a ZIP of every platform-size crop out. Tick any of the same social presets used by Crop/Resize (or "Select all"); each size is produced with the identical smart cover-crop as `/resize`'s preset path (no duplicated cropping logic), then bundled into one ZIP. `POST /api/graphics/export-social` (`{ imageDataUrl, presets: [presetId, ...] }`).
