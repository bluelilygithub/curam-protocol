...
3. **Glossary prep** — Vault translate model proposes / merges terms from intake + text skim + saved glossary (LLM engine only).
4. **Term locking (LLM engine only)** — `translateLlmService.lockRepeatedTerms`, backed by `translateQaChecks.detectRepeatedTermCandidates`, scans every paragraph for ordinary (non-brand) words/phrases that recur **mid-sentence** with a capital — `Warranty Schedule`, `Nominated Vehicle`, `Period`, `Make` — the kind of term too ordinary for the glossary-skim call to notice on its own, but exactly the terms that drift between chunks because nothing pins them down. Each candidate gets one canonical target rendering before translation starts and is merged into the working glossary.
5. **Translate** — chunked paragraph batches via `callModel` + glossary substitutions. Paragraphs that look like leaked code/template debris (e.g. a serialized object dump, an unresolved internal token) are detected (`translateQaChecks.isCodeLikeArtifact`) and copied through verbatim instead of being sent to the translator.
6. **Glossary consistency backstop (LLM engine only)** — `translateLlmService.enforceGlossaryConsistency`. Chunks translate in parallel with no shared state between them, so even a term everyone was told the same target for can still land two different ways in two different chunks. This scans every pair whose source contains a forced glossary term (user-declared or auto-locked in step 4) and re-translates, with an explicit "render X as Y" instruction, any segment whose target doesn't contain the canonical rendering.
7. **Hard sanity gate (deterministic)** — `translateQaChecks.hardSanityGate` on every source⟶target pair. If too many segments are identical to source (>30%), contain placeholders (≥2 or >5%), or are empty (>10%), the job **fails** with an error and a QA summary — no bilingual PDF.
8. **Review (optional)** — deterministic completeness runs first on **all** pairs (auto “Garbled / incomplete rows”); then the review model compares every pair side-by-side in batches for subjective issues; claim verification spot-checks a sample of “None flagged” segments.
9. **Client PDF** — `@react-pdf/renderer` bilingual PDF uploaded to complete the job.
