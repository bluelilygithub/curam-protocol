...
4. **Term locking (LLM engine only)** — `translateLlmService.lockRepeatedTerms`, backed by `translateQaChecks.detectRepeatedTermCandidates`, scans every paragraph for ordinary (non-brand) words/phrases that read as defined terms — `Warranty Schedule`, `Nominated Vehicle`, `Period`, `Make` — the kind too ordinary for the glossary-skim call to notice on its own, but exactly what drifts between chunks because nothing pins them down. A phrase qualifies on any of three signals: it recurs **mid-sentence** (≥2×, not just at sentence starts — plain capitalization isn't a signal on its own); it recurs as a **standalone paragraph** (≥2×, e.g. a field label like `Make` / `Model`); or it's immediately followed by a **definition marker** (`means`, `refers to`, `is defined as`, `has the meaning`) — this alone qualifies it even on a single occurrence, which is what catches a term that's only ever introduced once per definition clause (`"Warranty Schedule means..."`) and never referenced inline elsewhere. Each candidate gets one canonical target rendering before translation starts and is merged into the working glossary.
