...
**Confidence toggle (jobs table).** A **Confidence** button on any row with a QA summary reveals a heuristic High/Medium/Low badge under the status pill (click again to hide — per-row, not persisted). `computeConfidence()` in `TranslatePage.jsx` derives it client-side from the same `qaSummaryJson` the QA modal reads — hard/soft gate first, else the flagged-segment ratio (`uncertainTerms` + `polarityOrSentenceTypeIssues` + `audienceFlags` over reviewed pairs). No new model call and no server field — it's a label over existing QA output, not a distinct confidence score the model produced.

`garbledOrIncompleteRows` is deliberately excluded from that ratio — it's dominated by routine deterministic completeness flags (numbers/codes/headers identical to source, short segments), which the hard/soft gate already scores separately with its own thresholds (see **Completeness rules** below). Counting it a second time here pushed almost every real job's ratio past the Low cutoff regardless of actual translation quality — confirmed on real jobs, every one landed at the same 40%.
