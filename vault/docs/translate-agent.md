...
**Confidence toggle (jobs table).** A **Confidence** button on any row with a QA summary reveals a heuristic High/Medium/Low badge (with a %) under the status pill (click again to hide — per-row, not persisted). `computeConfidence()` in `TranslatePage.jsx` derives it client-side from the same `qaSummaryJson` the QA modal reads. No new model call and no server field — it's a score over existing QA output, not a distinct confidence value the model produced.

Scored by **weighted deduction from 97**, not a flag-count-vs-pair-count ratio — a ratio broke down on real reports: `uncertainTerms` carries one entry per *glossary term*, not per pair, so an 8-pair document can carry 10+ entries and any ratio-vs-total exceeds 1.0, bottoming every job out at the same fixed number (confirmed on a real report: 8 pairs, 10 `uncertainTerms`, landed at the same 40% as every other job). Deductions: `polarityOrSentenceTypeIssues` + `audienceFlags` (genuine meaning/grammar defects) at −15 each, capped −50; `uncertainTerms` (mostly glossary-nuance notes, not outright errors) at −3 each, capped −30; `softFail` at −20 flat. `hardFail` short-circuits to a flat 15%. `garbledOrIncompleteRows` stays excluded entirely — dominated by routine deterministic completeness noise already scored via hardFail/softFail (see **Completeness rules** below). Labels: ≥80 High, ≥55 Medium, else Low.
