...
Scored by **weighted deduction from 97**, not a flag-count-vs-pair-count ratio — a ratio broke down on real reports: `uncertainTerms` carries one entry per *glossary term*, not per pair, so an 8-pair document can carry 10+ entries and any ratio-vs-total exceeds 1.0, bottoming every job out at the same fixed number (confirmed on a real report: 8 pairs, 10 `uncertainTerms`, landed at the same 40% as every other job). Deductions: `polarityOrSentenceTypeIssues` + `audienceFlags` + `restructuredSentences` (genuine meaning/grammar defects) at −15 each, capped −50; `uncertainTerms` (mostly glossary-nuance notes, not outright errors) at −3 each, capped −30; `softFail` at −20 flat. `hardFail` short-circuits to a flat 15%.

`garbledOrIncompleteRows` is **not** uniformly excluded — only rows the deterministic gate itself produced (`check === 'deterministic_completeness'`, routine numbers/codes identical to source, already scored via hardFail/softFail) are treated as noise. Rows the LLM reviewer merges into that same array count as critical: a real report proved this matters — an entire untranslated paragraph and visible raw glossary syntax (`Ceci / C'est`) leaking into the output landed in `garbledOrIncompleteRows` with no `check` field, and excluding the whole category scored that document 70%. Labels: ≥80 High, ≥55 Medium, else Low.
