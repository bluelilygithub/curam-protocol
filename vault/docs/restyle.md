...
## The real cause of "scrape does nothing" / "paste lost its detected styles" — multer's default field-size limit

A third, more serious bug behind the same family of reports. Once CSS with inlined base64 background images (a normal outcome of the fix above) got sent from the client to `/process-css` inside the multipart `pasted` text field, it could exceed **busboy's default 1MB field-size limit** — multer/busboy's default behavior on overflow is to silently **truncate** the field, not reject the request. The truncated JSON then failed `JSON.parse()`, which an earlier version caught and *silently ignored*, leaving `pastedSheets = []`. Depending on what else was present, that either 400'd with "No stylesheets were provided" (scrape: nothing renders, looks like the button does nothing since `renderIframe()` is never reached) or just proceeded with an empty style list (paste: HTML still renders via the separate sanitize step, but with zero CSS applied — no colors/fonts/sizes to detect, since there's no CSS left to scan). Fixed two ways:
- `multer({ limits: { fieldSize: 25 * 1024 * 1024 } })` — 25MB comfortably covers a real page's CSS even with several inlined images.
- The `JSON.parse()` catch block no longer swallows the failure silently — it now 400s with a clear message and logs the field length server-side, so this class of bug can never again look like "the button does nothing" for no visible reason.

## Relative image paths — the other half of "images still don't render"

The CSP-driven inlining fix above only ever matched **absolute** `http(s)://` URLs. A scraped page's images are almost always **relative** (`src="/img/logo.png"`) — they're only ever meant to resolve against that page's own address, which is exactly why simply re-serving them from inside an `about:srcdoc` iframe (with no matching origin) could never have worked regardless of the CSP fix. `resolveUrl()` in `inlineImages.js` now resolves any non-absolute, non-`data:`/`blob:` reference against a `baseUrl` parameter before fetching — `/scrape-url` passes the scraped page's own URL for `<img>`/inline-style/embedded-`<style>` references, and each linked stylesheet's own URL (not the page's) for `url(...)` inside that specific file, since CSS relative paths resolve against the stylesheet's location. Plain upload/paste still has no way to know an origin to resolve against, so relative references there are left untouched, same as before — this fix is specific to the scrape path, where the origin is always known.

## "Build the preview" no longer requires a style file at all

After the embedded-`<style>` fix below still didn't fully resolve repeated reports of the button staying disabled, the requirement was removed outright rather than patched again: **CSS is optional.** The button is now enabled as soon as `htmlLoaded` is true, full stop — no dependency on `cssEntries` at all. `runBuildPreview()` skips the `/process-css` network call entirely when the style list is empty (`mergedCssRef.current = ''`) and renders the sanitized HTML as-is. This makes the feature correct by construction instead of depending on every CSS-detection path (embedded `<style>`, linked stylesheets, scrape) working — if none of them find anything, or the user simply doesn't add one, the preview still builds.

## Why "Build the preview" looked stuck — the CSS-detection part of the story
