...
| **Bank panel** | `bankPosture.js` + CDR | Merged row per bank: **Fit** (strong/fair/weak/unsuitable from capacity headroom + LVR/DTI + posture knobs), **per-bank indicative capacity**, documents list, live rate when CDR matches. Fit ≠ overall PASS/FAIL and is **not** a credit decision. PDF/UI include a Fit legend. |
