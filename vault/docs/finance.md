...
- **Purchase journal for every method, not just immediate**: a real gap in an earlier version — a >$300 asset posted no journal entry at all at purchase time, so the actual cash/credit-card outflow was never recorded until (and only partially, via the depreciation schedule) much later. Fixed: every asset, regardless of method, now posts **Debit Fixed Assets 1400** (a new asset account), **Credit `paidViaId` or Bank 1000**, for the full cost, immediately — a balance-sheet event, independent of the depreciation schedule which only ever affects book value afterward. `paidViaId` is a real field on both the Assets tab's form and (via the general Expenses form's existing "Paid via" field) the inline capital-asset flow, so a credit-card purchase can be settled just like a normal CC expense — `POST /assets/:id/cc-pay` mirrors `POST /expenses/:id/cc-pay`, since a non-immediate asset never creates a `fin_expenses` row to hang the existing CC-pay flow off of. The Register table's "Pay CC" action only shows for non-`immediate` assets — an immediate asset already has a linked `fin_expenses` row (via `immediateExpenseId`) and settles through the ordinary Expenses tab CC-pay flow instead.
...
- **Disposal**: `POST /assets/:id/dispose` (`{ disposedDate, disposalAmount }`) computes written-down value (`amount − accumulatedDepreciation`) and posts the balancing entry — the exact treatment depends on whether the asset was ever capitalized. `immediate` assets were fully expensed at purchase and never touched Fixed Assets, so disposal proceeds are pure profit/loss (Bank vs **Other Income 4100** or **Expenses 5000**). Every other method was capitalized (see purchase journal, above), so disposal must fully derecognize it — one entry: Debit **Accumulated Depreciation 1300** (clearing what's built up), Debit **Bank** (proceeds received), Credit **Fixed Assets 1400** (removing the original cost), plus a Credit to **Other Income 4100** (profit) or Debit to **Expenses 5000** (loss) to balance — never a standalone profit/loss line with the asset left sitting on the balance sheet forever.
