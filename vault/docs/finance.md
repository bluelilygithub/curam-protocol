...
- **Finance → Settings → "Vehicle & Home Office" section**: the ATO rate fields (`fin_vehicle_rate_per_km`, `fin_home_office_rate_per_hour`, each saved via `PUT /api/finance/settings` on blur), and the per-financial-year method lock for each claim type — a read-only summary of the current FY's locked method (or a one-time picker if that FY has no method set yet), plus a picker to lock the **next** financial year's method ahead of time (so reviewing in June can lock FY2026–27 before it starts). No per-entry method choice lives here or anywhere else — this is the only place a method is ever chosen.
...
- `finYearForDate()` (duplicated identically in `server/routes/finance.js`, `client/src/pages/FinancePage.jsx`, and `server/cron/financeRemindersCron.js` — keep all three in sync if it ever changes) resolves a date to `"YYYY-YY"` (FY starts 1 July). `nextFinYearLabel()` (client only, `FinancePage.jsx`) advances a FY label by one year for the "lock next FY ahead of time" control.
