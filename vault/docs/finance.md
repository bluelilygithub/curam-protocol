...
- **Low-value pool election — structural, not per-asset habit**: the ATO rule isn't "you may mix methods freely" — once an asset costing under `LOW_VALUE_POOL_MAX` ($1,000 ex-GST) is put in the low-value pool, every future asset under that threshold must be pooled too. This is enforced server-side, not left to memory: `fin_low_value_pool_election` (a `settings` row, JSON `{ elected, assetId, description, date }`) is set the first time a client-chosen `low_value_pool` method is saved for an eligible (`<$1,000`) asset. From then on, `POST /assets` ignores whatever method the client sends for any future `<$1,000` asset and forces `low_value_pool` — same "server resolves it, client can't override" pattern as the FY method locks elsewhere in this module. `GET /assets/low-value-pool-election` lets the client know the state; both the Assets tab's own form and the Expenses form's inline capital-asset fields call it, and once elected, the radio group doesn't even render for an eligible asset — a plain "Auto-pooled — per your earlier election" note replaces it, with a tooltip naming which asset triggered the election and when. Assets ≥$1,000 are never pool-eligible regardless of the election — they're always prime cost or diminishing value.
