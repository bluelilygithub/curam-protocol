...
- **Active generate provider now visible** — the Create tool shows a `Provider: FAL/Replicate · <model>` badge (from `GET /api/videos/status`'s existing `generate.provider`/`generate.model`), with a nudge to add `REPLICATE_API_TOKEN` when running on FAL, since FAL's default model (`fal-ai/minimax/video-01-live`) skews toward a stylized/CGI look for humans versus Replicate's `minimax/hailuo-2.3`. A help icon on the badge opens a modal explaining the FAL-vs-Replicate difference (which model each runs, why the look differs, billing is independent per-provider, how to switch via env var).
