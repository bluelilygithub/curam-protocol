...
6. **Full Stage 2 gate re-verified against a real deployed container, not just the DB.** After confirming the Railway "Staging" environment's Vault Application service had been left tracking `version-7` (a config gap from environment setup, not caught until pushing this stage — corrected to track `staging`), the full `milestone2.test.js` suite was run via `railway ssh` **inside the actual deployed staging container** (poppler-utils genuinely present via the Dockerfile, confirmed via `which pdftoppm`), not just against the staging Postgres from a local process. All 11 checks pass there, including the two that only ever skip-logged locally: scanned-PDF OCR and mixed-document OCR/page-order. Two small fixes needed once running for real: `contract_documents."ocrConfidence"` is `NUMERIC(5,2)`, which `pg` returns as a string, not a number — the test's `typeof === 'number'` assertion was wrong, not the write path; and the scanned fixture's obligation snippet checked `"AUD $1,200 per month"`, but Tesseract genuinely misreads the rendered `$` glyph as `3` at this fixture's render size (`$1,200` → `31,200`, confirmed by inspecting the raw OCR output directly) — an authentic OCR imprecision the spec's own guardrails already anticipate (normalized matching, `verificationStatus`), not a pipeline defect; the snippet now checks `"1,200 per month"` instead.

**Stage 2 follow-up pass (post-review, before Stage 3):**

1. **Production-path guard added for the `pdf-parse` corruption (item 4 above), not just a test-side retry.** Investigated the actual failure characterization in depth before deciding a fix, per the specific ask: ran ~80 combined extraction attempts across this investigation (40 in-process with no interleaved I/O — 0 failures, 0 content mismatches; 20 with real Postgres I/O interleaved immediately before each extraction, matching `ingestDocument`'s own shape — **16/20 failed**; multiple fresh-process single-call trials). **Every single failure, with no exception, was a loud thrown `InvalidPDFException`** (`Invalid PDF structure` / `Unknown compression method in flate stream` / `Bad FCHECK in flate stream`) — **never once a silent, successfully-returned-but-wrong `extractedText`.** This matters directly for the concern raised: a corrupted extraction can't become "ground truth" for spans/quote-verification/lineage, because the corrupted case never returns successfully in the first place — `ingestDocument`'s existing try/catch already treats any extraction exception as `status='failed'`, so this was already structurally impossible to reach as silent corruption, not merely improbable. Interleaved real I/O (not raw call count) is the dominant trigger — consistent with a legacy `setTimeout`-based scheduler racing the event loop, not a content-dependent bug. Given failures are loud but frequent under realistic I/O patterns (up to 80% in the worst tested case), a real document should not need manual re-upload just because it landed on an unlucky attempt: `ingest.js` now wraps the extraction call in `extractPdfOrDocxWithRetry()` — up to 3 attempts, only for this exact error signature, each attempt logged (`console.warn`), final exhaustion still calls `markReviewFailed` and throws exactly as before. This is a **retry for reliability, not a corruption-detection/repair mechanism** — none was needed, since no corrupted-but-successful case exists to detect. `translate.js` is untouched and remains equally exposed (same Suggestions-inbox alert covers it); noted again here since it's genuinely the more exposed of the two given Translate's much higher real-world PDF-upload volume.
2. **Confirmed Stage 1's boot verification actually ran on the `staging` branch, not `version-7`, for the first time with this push** — the branch-tracking gap (item 6 above) meant every earlier "staging" boot check was silently checking `version-7`'s code the whole time. Re-verified directly: `railway logs` on the current deployment shows `[db] PostgreSQL connected` → `[contract-review] contractType enum matches playbooks.js keys` → `Vault server running on port 3001` → `[db] Schema ready`, with no crash — the enum-sync assertion sits unconditionally before `server.listen()` and calls `process.exit(1)` on any mismatch, so this sequence completing is only possible if the assertion passed and the migration section completed (see the reasoning already given when this line was first confirmed). Production's own trigger branch was checked in the same pass and confirmed still `version-7`, untouched.
3. **Segmentation: realistic hierarchical numbering + a sequence-continuity check.** The deep-nesting fixture originally used fully-qualified compound labels (`2.2(a)(i)`) as if each were its own heading — unrealistic; a real contract's `(a)`/`(i)`/`(ii)` are sub-items of their parent numbered clause, not top-level clauses. Rewrote the fixture with proper hierarchy (`4.1 Payment Terms.` as the heading, `(a)`/`(i)` lines as body text within it) and **removed the compound pattern from `HEADING_PATTERNS` entirely** — sub-items no longer match as headings at all. Added `continuesSequence(candidate, last)` in `segmentation.js`: a candidate heading is only accepted if it's the first one, continues the same top-level section with the next minor number, or starts the very next top-level section — rejecting a same-major "restart" with no minor (an embedded numbered list: `1. / 2. / 3.` appearing inside an already-open section) and rejecting a non-adjacent major (a cross-reference like `"Section 8 shall survive termination"` that merely looks like a heading). Two new unit tests (`testSequenceCheckRejectsCrossReference`, `testSequenceCheckRejectsEmbeddedNumberedList`) cover both directly against `heuristicSegment()`. **A real, separate bug surfaced while verifying this against actual PDF-rendered output** (not just the raw fixture string): `extractFromPdf`'s own paragraph-reconstruction merged the fixture's title line with its first heading paragraph into one line that no longer matched any heading pattern — and `heuristicSegment()` was **silently dropping all text before its first recognized heading**, which would lose a real contract's letterhead/preamble/title entirely, not just a fixture artifact. Fixed by capturing any pre-heading text as its own leading clause (`numberLabel: null`) rather than discarding it. The fixture itself also needed a genuine preamble paragraph between the title and clause 1.1 (a real MSA always has one — party identification, recitals) so the two reliably render as separate paragraphs; `expectedNumberLabels` now starts with a leading `null` entry for that captured preamble clause.
4. **Expected obligations now carry the full field set** (`sourceQuote`, `obligor`, `timingShape`, `anchorEvent`, `offsetDays`, `amount`, `currency`, `rrule`, `absoluteDate` where applicable) — renamed from the earlier `snippet` field, same mechanism: an exact, hand-picked substring from the fixture's own text, located via `extractedText.indexOf(sourceQuote)`. To be explicit about how test 11 locates obligations, since there is no obligation-extraction code to do anything cleverer: **`sourceQuote` is chosen by hand while authoring the fixture, not derived from anything** — it's the literal source text a real obligation-extraction stage would eventually need to quote and verify, written down now so Stage 4 has a fixed target to check against rather than inventing its own.
5. **OCR scheduler**: `OCR_LANGS` changed to `'eng'` only (Contract Review only handles English-language contracts; the other 7 languages `translate.js` loads were pure unused overhead here) and an idle-shutdown timer (5 minutes of no OCR activity) now terminates the worker pool and lazily re-initializes on next use, rather than holding 4 Tesseract workers open for the process's entire lifetime.
6. **Scanned fixtures degraded deterministically** — the scanned page and the mixed document's scanned exhibit page now get a fixed rotation (1.3°), fixed Gaussian blur (σ=0.6), and a fixed downscale/upscale round-trip (0.6× then back), simulating a real imperfect scan rather than a pristine rendered page; no `Math.random` or other non-deterministic input anywhere. **Determinism was NOT actually true until a separate fix**: `pdf-lib`'s `PDFDocument.create()` stamps `/CreationDate`/`/ModDate` with the current wall-clock time by default — confirmed directly (sha256 of two independent generations of the same fixture differed) before pinning both to a fixed constant date on every `PDFDocument.create()` call, after which PDF fixtures are byte-identical across runs (re-confirmed). The two DOCX fixtures' raw bytes still differ run-to-run (JSZip's default per-entry timestamp, from the shared `buildDocxFromParagraphs` helper reused from `documentRedaction` — not modified, to avoid touching shared code for a cosmetic concern) — but their **extracted content is confirmed byte-identical** (hashed the `mammoth`-extracted text across two runs, identical both times), which is what `sourceQuote` matching actually depends on; the timestamp difference has no effect on anything the pipeline or tests read. On the specific question of whether the earlier OCR-snippet fix loosened the assertion or made it tolerant: **neither** — the assertion is exactly as strict as before (still requires an exact substring present in the OCR'd text); only the *choice* of which substring to check changed, moving off a character Tesseract genuinely misreads at this fixture's render size onto one it reads correctly.
...
**Correction, same session — the child-process fix above was wrong, caught by testing it on the real deployed runtime.** Re-running the full suite via `railway ssh` after redeploying the child-process fix showed 5/5 failures, including a *standalone* test of `extractInChildProcess.js` with no DB, no wrapper, nothing else running (10/10 failures). Investigated why the container behaved completely differently from local: **the container runs Node 20; this dev machine runs Node 24.** Testing directly on Node 20 in the container revealed the real failure shape is nothing like the event-loop-timing-collision theory the child-process fix was built on — it's a **cold-start bug**: the first 1-2 extraction calls in any freshly-started process fail reliably; every call after that, in the same warm process, succeeds reliably (confirmed twice: 2/2 cold fails then 0/18 warm; repeated with the same split). The DB-adjacency correlation observed earlier on Node 24 was a real but different phenomenon on a Node version that isn't what's actually deployed.

This makes the child-process "fix" actively wrong for production: isolating each extraction into a fresh child process guarantees every single one is cold, which is exactly why it measured 10/10 failures standalone in the container — the fix reliably triggered the bug it was meant to prevent. **Reverted.** The correct fix for a long-lived server process (which cold-starts once, at boot, not once per document) is to burn the cold-start failures against a throwaway warm-up extraction at module load — `ingest.js`'s `warmUpExtractor()` now builds a tiny one-page PDF via `pdf-lib` and extracts it up to 3 times at boot (fire-and-forget, never blocks startup), so the 1-2 cold failures happen against a dummy document instead of a real user's. `extractInChildProcess.js` deleted. Extraction runs in-process again, wrapped in the same bounded retry as a backstop for whatever residual rate remains after warm-up.

Updated the Suggestions-inbox alert again to reflect this — Translate's live production route is almost certainly affected far less severely than the first characterization suggested, since its server process also only cold-starts once at boot, not per-upload; the actual fix needed there (if anyone chooses to apply it) is the same warm-up-at-boot pattern, not process isolation.

The lesson worth stating plainly: the first "structural fix" was reasoned entirely from local (Node 24) behavior and looked convincing — clean before/after numbers, a plausible mechanism — but was never tested against the actual production runtime before being shipped. It was only caught because the instruction to re-verify via `railway ssh` inside the real container was followed through on before declaring the work done.

**Second correction, same investigation — the warm-up-only fix was also incomplete, caught the same way.** Testing warm-up specifically on Node 20 (the real target) revealed it fixes cold-start but not the whole picture: after a successful warm-up, running the *exact* `ingestDocument` shape (a DB query awaited immediately before extraction) 20 more times **still failed 9/20 (45%)**. Two genuinely separate failure modes are both real on the actual production runtime:

1. **Cold-start** — the first 1-2 extraction calls in any freshly-started process fail reliably (fixed by warm-up).
2. **DB-adjacency** — even in an already-warm process, a DB query awaited immediately before the extraction call fails at a real, material rate (not fixed by warm-up; this is the failure mode the first, reverted child-process fix was actually responding to, correctly, before it was undone for the unrelated and also-correct reason that a fresh-process-per-call guarantees mode 1).

Neither of the two prior fixes addressed both modes. The actual fix needs a process that is **both** warm **and** never shares an event loop with DB I/O, for its entire lifetime: a **persistent, long-lived worker process** (`server/services/contractReview/extractWorker.js`, forked once via `child_process.fork()` and kept alive/reused for every extraction — same lazy-singleton shape as `ocrScheduler.js`'s worker pool, just one process instead of four). It warms itself up once at fork time (embedding the same retry-until-success loop), then handles every real extraction request over the IPC channel for as long as it stays alive — no DB call is ever adjacent to one in that process, and it's never cold after its own first request. `ingest.js`'s `extractPdfOrDocxWithRetry` now routes through `extractViaWorker()` (a small promise-based request/response wrapper keyed by a request id, with its own timeout), keeping the bounded retry as a backstop layer on top.

Measured against the exact same `ingestDocument`-shaped trigger (real `ContractService.addDocument` + `ingestDocument`, 20 iterations, a real DB row read/write around each call): **0/20 failures**, both locally and — the number that actually matters — on the real Node 20 container via `railway ssh`. One internal retry fired on the very first call in the local run (the worker's own first request, absorbing its remaining cold-start slack), zero issues on every call after.

The test harness itself also needed a correction as a direct consequence: `milestone2.test.js` previously ran each fixture's `ingestDocument` call in its **own fresh subprocess** (`ingestSegmentSubprocess.js`, built during the first, wrong characterization to dodge a "many calls in one process" theory that turned out not to be the real bug). That design guarantees a cold start on every single fixture — exactly the condition the persistent worker exists to avoid — so it was actively fighting the real fix. Removed entirely: the test file now calls `ingestDocument`/`segmentDocument` directly, in the one shared test process, for every fixture — matching how a real long-lived server actually behaves (one cold start, ever, then reliably warm), which is also the only way the persistent worker's design can actually be exercised correctly by a test.

Updated the Suggestions-inbox alert a third time with the complete, two-mode characterization and the concrete fix (the same persistent-worker pattern) for `translate.js`, which is exposed to mode 2 on every real upload today (DB work immediately around each extraction call, identical to `ingestDocument`'s shape) — likely the more materially-affected of the two features in practice, given Translate's real-world upload volume.

Re-ran the full suite one final time after this fix: locally and via `railway ssh` inside the redeployed container.

**Fourth round — stopped chasing pdf-parse call patterns, replaced pdf-parse entirely for PDFs.** Per explicit instruction after the third fix (the persistent worker) still showed a non-trivial residual failure rate in a full realistic test run despite measuring 0/20 in an isolated repeated-call test: three structural fixes had now been tried (process isolation, warm-up, persistent worker), each one closing exactly one failure mode while a different one stayed open or a new residual appeared, and raising the retry count again would only have hidden the problem further rather than fixed it. **The actual conclusion: the problem is `pdf-parse`'s bundled `pdfjs` (v1.10.100) itself — an old, buggy, unmaintained decoder — not anything about how `ingest.js` called it.** No amount of retrying, warming up, or isolating fixes a bug in the library.

**Reversing the original Stage 2 decision to reuse `translateExtract.js`'s `extractFromPdf` for PDFs.** Contract Review's PDF extraction now goes through poppler's `pdftotext` directly (`server/services/contractReview/pdfTextExtract.js`) — the same binary already installed via `poppler-utils` in the Dockerfile for `pdfRasterize.js`'s `pdftoppm`, so this needed zero new dependencies. `pdftotext -layout` output is split on its form-feed (`\f`) page separators; each page's paragraphs are grouped with `translateExtract.js`'s own exported `splitParagraphs()` (reused, not reinvented — same blank-line convention as the rest of the codebase); `scannedCandidatePages` uses the identical density rule `extractFromPdf` uses (a page's total paragraph character count under 20 → likely scanned). **DOCX is untouched** — still `extractForTranslate`/mammoth via `translateExtract.js`, since DOCX was never part of the pdf-parse problem.

Removed as part of this reversal: `extractWorker.js` (the persistent-worker process), the bounded-retry loop and `KNOWN_PDF_PARSE_FLAKE` regex in `ingest.js`, and `smokeSet/ingestSegmentSubprocess.js` (the earlier per-fixture subprocess isolation, no longer needed since nothing about pdftotext requires it). `ingest.js`'s extraction path is now a plain, direct call — no special-casing at all.

**A real, separate fixture bug surfaced immediately when re-testing against pdftotext**, the same class of "reads correctly under one extractor, not the other" issue Stage 2 has hit before: `pdftotext -layout`'s own paragraph-break detection requires a wider vertical gap between lines than `pdf-parse`'s y-gap heuristic did — with the fixture's original single-line-height paragraph gap, `pdftotext` printed every paragraph as its own line but with **no blank line** between them (poppler's own blank-line insertion threshold not met), collapsing the whole deep-nesting fixture into one giant paragraph under `splitParagraphs()`'s blank-line convention. Confirmed directly by inspecting raw `pdftotext -layout` output. Fixed by widening `buildTypedPdf`'s inter-paragraph gap to a full two-line-height gap for every paragraph (previously only the title got this, for the equivalent pdf-parse-side issue) — matching how a real Word/Adobe-exported document is actually spaced, which is also just a better fixture regardless of which extractor reads it.

Re-ran the full suite after this fix: **zero pdf-parse-signature failures of any kind** — not "recovered via retry," genuinely absent, for the first time in this entire investigation. Confirmed locally (this sandbox has `pdftotext` on PATH via mingw64, unlike `pdftoppm` — PDF-dependent tests that previously had to skip locally now run for real) and via `railway ssh` inside the redeployed container.

Suggestions-inbox alert updated a final time recommending the identical `pdftotext` switch for `translate.js`'s own PDF path — likely the more materially-affected of the two features in practice, given Translate's real-world PDF-upload volume. Not fixed in `translate.js` — out of scope for this feature.

**Stage 3/4/6 build (approved deviations — Stage 3 analysis pipeline, Stage 4 obligations, Stage 6 UI/routes, built together per instruction):**

1. **`playbooks.js` and `contract_clause_types` seeding already existed from Stage 1** (confirmed by direct exploration before writing any new code) — not built fresh here. New for this stage: `coverageExpectations.js` (pipeline stage 11's own config — which of the 14 v1 clause types a given `contractType` is expected to contain at all, separate from `playbooks.js`'s risk-position config), `grounding.js` (stage 10's shared quote→span verification, used by definitions/obligations/summary rather than three near-duplicate implementations), and `rawOutputs.js` (one insert helper for `contract_review_raw_outputs`, called from every LLM-calling stage).
2. **`prompts/v1/index.js`** — all Stage 3/4 prompts in one file, one `PROMPT_VERSION` string, stored on `contract_reviews.promptVersion` per the spec's explicit instruction to treat the whole set as one version. Per-stage `modelId` is still recorded separately per call.
3. **`confirmParty` now also sets `isUser=TRUE`** (clearing it on every other party on the contract first). Not explicit in the original Stage 1 signature, but risk scoring (stage 7) needs to resolve `(contractType, role)` for *the confirmed party*, and the spec's own party-reconciliation note already treats "confirmed" and "is the user" as the same act ("the user's own role is a row here"). Reason for the deviation: without this, nothing in the schema ever set `isUser=TRUE` at all — `addParty`'s `isUser` param is only used by direct API callers (Stage 6 has none that pass it), so relying on it alone would leave every pipeline-extracted party permanently `isUser=FALSE`.
4. **Pipeline pause/resume is two explicit steps, not automatic.** `confirmParty` stays a pure confirmation action; a separate `ContractService.resumeReview` / `POST /reviews/:id/resume` continues stages 5-11. Considered making `confirmParty` auto-resume any paused review on the contract (closer to the spec's prose "unblocks the review"), but `resumeAfterRoleConfirmation`'s stages 5-11 all `INSERT` (never upsert) — auto-resuming twice by accident (e.g. a UI double-click, or a test calling both paths) would silently duplicate every definition/obligation/summary point on an already-complete review, directly violating the immutability guardrail. Rejected as riskier than the two-step version for no real benefit; `/reviews/:id/resume` is an addition to the spec's illustrative (not exhaustive) route list, same precedent as Stage 1's extra `contract_events` types.
5. **`resumeAfterRoleConfirmation` refuses to re-run against a `status='complete'` review**, returning `{status:'complete'}` immediately instead — a direct guard for the same duplication risk described above, found while designing item 4, not from an observed bug.
6. **Risk scoring runs per-clause, one model call each** (not batched), matching the spec's stage 7 description ("each clause + injected definitions... + playbook") literally rather than optimizing for fewer calls — acceptable for a smoke-set-sized document; a real high-volume optimization (batching several clauses per call) is a reasonable future change, not made here.
7. **Coverage `could_not_assess`** is set whenever a review has zero clauses, or used the `llm` segmentation fallback and classified nothing at all — a narrower, more mechanical trigger than "OCR failure" specifically (there's no direct OCR-confidence threshold wired into this decision), but it's the same distinction the spec asks for: genuinely-absent vs. couldn't-tell.
8. **Milestone 3/4 comparison scripts are LLM-based, not deterministic** (unlike milestone1/2, which never call a model). Assertions are written to tolerate normal model variance where the spec itself allows it (`riskLevel='unclear'` is a valid answer; must-not-flag tolerates at most one false positive per contract, per instruction) while still failing hard on anything the spec guarantees mechanically: exact `contractType`/party-name matches, grounding honesty (a `verified_*` row's span must actually match its quote), obligation timing shape with no invented `absoluteDate`, and the role-flip fixture's role-sensitive flags actually inverting. Both scripts require `ANTHROPIC_API_KEY` in addition to the existing `TEST_DATABASE_URL` gate, and create their own `vault_models` settings row per test user (a fresh test DB has no admin user for `getModelsForUser` to fall back to).
9. **Role-flip tested via a second, separate contract, not a re-review of the same document.** Party confirmation is deliberately sticky per contract (spec: re-reviewing a new draft shouldn't re-ask a question already answered) — that design means a *second* review of the *same* document automatically skips `awaiting_role_confirmation` and reuses the first run's confirmed party, so it can't be used to test a different role on purpose. Uploading the same fixture bytes into a fresh contract and confirming the other party there instead tests the same risk-scoring role-sensitivity without fighting that (correct) stickiness.
10. **`exportIcs` only resolves `anchorEvent='effective_date'`** to a concrete date (via `contracts.effectiveDate` + `offsetDays`) — `renewal_date`/`invoice_date`/`termination`/`custom` anchors have no stored anchor date anywhere in the v1 schema (no `renewalDate` column, etc.), so those stay unresolved and are simply not included in the `.ics` file rather than guessing a date. This matches the spec's "unresolvable relative obligations... listed separately in the UI, not silently dropped from the export" — the obligations view (not this file) is where they're listed; nothing in the export step invents a date to force inclusion.
11. **`listObligations` returns `verificationStatus='failed'` obligations too**, tagged `derivedStatus='unverified'` instead of a time-based bucket, rather than omitting them outright — needed so the UI can show them "visibly separated" per the spec's UI section, while `exportIcs`/`linkObligationToTask`'s effective exclusion still happens (the route/UI layer skips acting on `unverified` rows for Add-to-Task/ICS, matching "excluded... until the user confirms it via a correction").
12. **`promoteReviewToContract` gates `contractType` promotion on `kind='base'` (or an explicit contract-level correction), same as the key-term fields** — the spec's base-vs-amendment distinction is written specifically about key terms, but an amendment's own contract-type re-detection is redundant at best and a risk of silently overwriting the base agreement's already-correct type at worst if a later document is mis-detected. Treated as falling under the same "amendments don't overwrite real values with a redundant re-detection" principle, not tested directly in the milestone4 smoke run (which only executes a base document) — flagged here rather than silently assumed.
