...
- **Branch suggestions** — post-stream `POST /api/chat/branches` call (fires alongside suggestions after each stream); model used = `branch_eval_model` setting if set, else active chat model, else light fallback; routes to Gemini/DeepSeek/Anthropic correctly; skips `quick`-type projects and short responses (<400 chars); `POST /api/chat/sessions/seed` creates the pre-seeded session; `BranchSuggestions.jsx` renders "Explore deeper" chips below follow-up chips; clicking navigates to new session in same project. **Branch evaluation model** configurable in Settings → Models (`branch_eval_model` key) — set to a capable model to ensure branches trigger with flash/economy chat models
