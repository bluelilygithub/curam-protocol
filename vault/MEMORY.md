...
- **Precious Metals** — Metals tab in Shares page; `metal_purchases` table; `server/routes/metals.js`; spot price via Finnhub `OANDA:XAU_USD` → AUD (`getGoldSpotAud()` in `marketData.js`); coin count × oz helper auto-fills total; summary cards + per-purchase P&L vs spot; gated by `requireFeature('shares')`
