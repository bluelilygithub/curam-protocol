...
- **Quick chat** (`/chat` route, `<ChatPage general />`) — project-free chat; sessions stored with `projectId = null`; **Quick chat** section in sidebar with expandable session list and "+" button
- **Home Continue** — `/` shows cross-workspace recents (`GET /api/chat/recent`); **New chat** routes instantly (no modal); sidebar **Recent** (5) + chat header **Home** resume latest
- **Chat UX (2026-07)** — instant new chat; empty-state **Context** picker (Quick vs project); project row **⋯** overflow (archive confirms); `/history?projectId=` filter; client touchpoint form hidden on chat routes
...
## client/src/utils/apiClient.js is a raw fetch wrapper, NOT axios
`get`/`post`/`put`/`patch`/`delete` return the raw `Response` object — no `.data`, no auto-throw on non-2xx (only throws/redirects on 401). Every call site must `await res.json()` and check `res.ok` itself. **Before writing any new client page**, check this shape (or copy an existing page like `GmailIntelPage.jsx`) instead of assuming axios convention — missed once while building Expense Review page below, caught before ship. Also: file/attachment links must go through `api.get` → `res.blob()` → `URL.createObjectURL`, never a plain `<a href="/api/...">`, since auth is a Bearer header not a cookie.

## Expense Review (Invoice/Bill review queue) — built 2026-09-17
Additive pipeline closing the gap between Inbox Intel flagging an invoice email and a Finance expense being recorded. Feature flag reused: `gmailIntel` (not a new flag).
- **Schema:** `expense_review_queue` (server/db.js) — `gmailMessageId` unique per `userId` (dedupe key). Column `"s3Url"` is actually a **local disk path** under `UPLOAD_DIR/invoices/{year}/{month}/...` — no S3 in this app; name kept to match original spec, commented in db.js.
- **Shared pipeline:** `server/services/expenseReviewService.js` (`runPoll`) — used by both the backfill script and the hourly cron so they never diverge.
- **Backfill:** `server/scripts/backfillInvoiceReview.js` — mirrors `migrateFinClientsToClients.js` pattern (`--dry-run`, per-row log, summary counters). 6-month window, all Gmail-connected users.
- **Cron:** `server/cron/expenseReviewCron.js`, hourly, first-run-per-user = 6-month lookback (via `settings` key `expense_review_last_polled_at`, no new job-state table).
- **Reused, not reimplemented:** extracted `findFirstPdfAttachment`, `extractInvoiceFromPdf`, `classifyEmailBatch` out of `server/routes/gmail.js` (used by both the original `/threads/:threadId/extract-invoice` route and this new pipeline — same output, single code path). Extracted `createExpenseRecord` out of `server/routes/finance.js` `POST /expenses` (insert + journal entry) for the same reason — `server/routes/expenseReview.js`'s bulk-create/bulk-mark-paid call it directly rather than duplicating the accounting SQL.
- **Design decision — queue `paid`/`paidDate` is NOT a Finance write-back:** `fin_expenses` has no paid concept (an expense row already represents money paid, posted via journal entry at creation); `fin_invoices`' paid lifecycle (`status`/`"paidAt"`) is for receivables, unrelated. The queue's `paid` flag is purely local "has this bill actually been settled outside the app" tracking, documented inline at `server/routes/expenseReview.js`'s `bulk-mark-paid` route. If Finance itself ever needs this, it's a separate schema change — not done here.
- **UI:** `client/src/pages/ExpenseReviewPage.jsx` at `/expense-review`, nav item "Invoice Review" under the same section as Inbox Intel.
- **Live status (2026-09-17):** cron deployed and running hourly — relied on for going-forward capture. Full historical backfill to July 1, 2026 abandoned by user decision, not a bug left unfixed blind: `classifyEmailBatch` (server/routes/gmail.js) sends the whole needs-classification list in one model call with no chunking, and a 100-155 thread backlog (July 2026→now) overflowed the token ceiling twice in a row (empty/truncated deepseek response even after the existing retry-with-larger-budget logic) — that batch got zero classifications, silently. Fixed separately: real Gmail `pageToken` pagination via `fetchAllInboxEmailsSince` (previously a single fetch-then-filter page silently missed history beyond ~100 threads — this part IS fixed and confirmed working, 3 pages / 155 threads fetched correctly). The remaining gap is purely the classify-batch size cap — logged as suggestion #25, fix is chunking into ~25-thread groups if a historical sweep is wanted later. Initial real backfill run's 3 captured rows (Taumata invoice, Anthropic receipt, and one bad row — a Westpac bank statement misidentified as an invoice via the first-PDF-in-thread heuristic, suggestion #24) are being kept as-is, used as learning/reference data rather than redone.
- **Not yet manually tested:** portal UI walkthrough (edit+create-expense confirming edited values land not raw extraction, mark-paid confirming no fin_expenses/fin_invoices side-effect, reject confirming no Finance write) — still open, do before trusting the UI end to end.
