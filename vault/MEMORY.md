...
- **Branch suggestions** — LLM can suggest up to 3 branch chats at end of a response for complex topics; `BRANCH_INSTRUCTION` in system prompt Block 1 for all non-`quick` project chats; server strips `[BRANCHES]...[/BRANCHES]` from stored content and emits via SSE; `POST /api/chat/sessions/seed` creates pre-seeded session; `BranchSuggestions.jsx` renders "Explore deeper" chips below follow-up chips; clicking navigates to new session in same project
