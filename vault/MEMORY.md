...
- **Not yet verified live** (local dev broken per usual) — full manual test plan agreed but not yet run on Railway: dry-run backfill → real backfill → edit+create-expense (confirm edited values land, not raw extraction) → mark-paid (confirm no fin_expenses/fin_invoices write) → reject (confirm no Finance write) → cron incremental run (confirm dedupe on `gmailMessageId`, no re-flagging backfilled rows).
