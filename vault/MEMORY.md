...
- **Branch suggestions** — post-stream `POST /api/chat/branches` call (fires alongside suggestions after each stream) uses the session's own model (Gemini/DeepSeek/Anthropic routed correctly) to decide if branching is warranted and generate up to 3 branch objects; skips `quick`-type projects and short responses (<400 chars); `POST /api/chat/sessions/seed` creates the pre-seeded session; `BranchSuggestions.jsx` renders "Explore deeper" chips below follow-up chips; clicking navigates to new session in same project
